...
@pytest.mark.parametrize(
    "test_app",
    [{"buildername": "html", "srcdir": "doc_test/doc_case_custom_classes", "warnings": sys.stderr}],
    indirect=True,
)
def test_case_classes(test_app):
    ...
    assert '<table class="test-case custom-case' in html
    assert html.count('<tr class="test-action') == 2
    assert html.count('<td class="test-action">') == 1
    assert html.count('<td class="test-action custom-action">') == 1
    assert html.count('<td class="test-reaction">') == 1
    assert html.count('<td class="test-reaction custom-reaction">') == 1
